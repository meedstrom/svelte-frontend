...
export function load({ params }) {
    const post =
        ...
        [...stored(pubMeta).values()].find(
            (post) => post.slug === params.first
        ) ??
        [...stored(privMeta).values()].find(
            (post) => post.slug === params.first
        )
    if (post) {
        // User is logged in
        if (stored(allowedTags).length > 0) {
            let content =
                stored(pubPosts).get(post.pageid) ??
                stored(privPosts).get(post.pageid)
            ...
            return { post, content, id: post.pageid, dailies, dailySlugs }
        }
        // User is not logged in (and probably trying to access a known pageid)
        else {
            redirect(307, `/${post.pageid}/${post.slug}`)
        }
